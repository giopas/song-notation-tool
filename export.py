...
def _body_lines_for_width(doc: dict, instruments) -> list[str]:
    ...
    out += chords_mod.sheet_lines(doc, width=DEFAULT_TXT_WIDTH)
    return out
...
def _chord_sheet_lines(doc: dict, width: int = DEFAULT_TXT_WIDTH) -> list:
    """The chord-shape block, with its own heading — printed once, at
    whichever end of the chart the document asks for."""
    body = chords_mod.sheet_lines(doc, width=width)
    if not body:
        return []
    def div(ch):
        return "-" * width
    return [div("-"), "  CHORDS", div("-"), ""] + body + [""]
...
def _build_pdf(doc: dict, instruments, orient: str, scale: float,
    ...
    def draw_chord_sheet(cy):
        ...
        so a diagram and a lick read as the same kind of thing. Returns
        the y it left off at."""
        body = chords_mod.sheet_lines(doc, width=max(20, int(COL_W_TEXT / CHAR_W)))
        if not body:
            return cy
        if cy < FOOTER_H + LINE_H * (len(body) + 2):
            cy = next_column()
        color(0.16, 0.24, 0.42)
        txt(col_x0(), cy, "CHORDS", sz=HEAD_SZ, bold=True)
        cy -= LINE_H + 2 * S
        color(0, 0, 0)
        for ln in body:
            if cy < FOOTER_H + LINE_H:
        ...
        if chart_rows:
            ...
            if beside:
                ...
                for ln in lyric_lines:
                    txt(lyric_x, lyric_cy, ln, sz=MONO_SZ)
                    lyric_cy -= LINE_H
                cy = min(cy, lyric_cy)
                lyric_lines = []
            cy -= 2 * S
